...
export const ImageUploader = {
  render: (existingImages = []) => {
    const slots = Array.from({ length: 10 }).map((_, idx) => {
      const img = existingImages[idx] || null;
      return `
        <div class="relative group h-32 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/40 p-2 flex flex-col items-center justify-center overflow-hidden hover:border-brand-500 transition-all">
          ${img ? `
            ...
            <button class="absolute top-2 right-2 p-1 rounded-lg bg-rose-600 text-white opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          ` : `
            ...
            <input type="text" placeholder="ইমেজ URL" class="mt-1 w-full px-1.5 py-0.5 text-[10px] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded text-center outline-none" />
          `}
        </div>
      `;
    }).join("");
