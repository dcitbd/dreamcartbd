...
export const CategoryManager = async () => {
  ...
  return `
    <div class="min-h-screen flex bg-slate-50 dark:bg-luxury-dark font-bengali">
      ${Sidebar?.render ? Sidebar.render("/admin/categories") : ""}
      ...
      <main class="flex-1 p-6 sm:p-10 max-w-7xl mx-auto overflow-y-auto">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 mb-8 border-b border-slate-200 dark:border-slate-800">
          ...
          <button id="add-main-cat-btn" type="button" class="btn-primary py-2.5 px-5 text-sm flex items-center gap-2 shadow-md shadow-brand-500/20">
        ...
        <div class="space-y-6">
          ${categories.length === 0 ? `
            <div class="glass-panel p-12 text-center rounded-3xl text-slate-400 border border-slate-200 dark:border-slate-800">
              <i data-lucide="folder-x" class="w-12 h-12 mx-auto mb-3 opacity-50"></i>
              <p class="text-sm">কোনো ক্যাটাগরি পাওয়া যায়নি। নতুন ক্যাটাগরি তৈরি করুন।</p>
            </div>
          ` : categories.map(main => `
            <div class="glass-panel p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
              <div class="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-800">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <h3 class="text-base font-bold text-slate-900 dark:text-white">${main.category_name || main.name || "ক্যাটাগরি"}</h3>
                    <span class="text-xs text-slate-400">ID: ${main.category_id || "N/A"} | Slug: ${main.slug || 'n/a'}</span>
                  </div>
                </div>
                <div class="flex items-center gap-2">
                  <button type="button" class="btn-secondary text-xs px-3 py-1.5 flex items-center gap-1">
                    <i data-lucide="plus" class="w-3.5 h-3.5"></i> সাব-ক্যাটাগরি
                  </button>
                  <button type="button" class="p-1.5 text-slate-400 hover:text-brand-500 transition-colors">
                ...
                ${(main.subCategories || []).map(sub => `
                  <div class="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800">
                    <div class="flex items-center justify-between mb-2">
                      <span class="text-sm font-bold text-slate-800 dark:text-slate-200">${sub.name || sub.category_name || "সাব-ক্যাটাগরি"}</span>
