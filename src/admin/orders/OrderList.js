...
export const OrderList = async (params = {}) => {
  ...
  const statusTabs = [
    ...
    { label: "কুরিয়ারে পাঠানো", key: "shipped", count: orders.filter(o => o.order_status === 'shipped').length },
      ...
      <main class="flex-1 p-6 sm:p-10 max-w-7xl mx-auto overflow-y-auto">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 mb-6 border-b border-slate-200 dark:border-slate-800">
          <div>
            ...
            <p class="text-xs text-slate-500 mt-1">অর্ডার প্রসেস করুন, কুরিয়ার ফ্রড চেক করুন এবং বাল্ক শিপমেন্ট পাঠান</p>
          </div>
            ...
            <table class="table-modern text-xs">
              <thead>
                <tr>
                  ...
                  <th>স্ট্যাটাস চেঞ্জার</th>
                  <th class="text-right">অ্যাকশন</th>
                </tr>
              </thead>
              <tbody>
                ${filteredOrders.length === 0 ? `
                  <tr><td colspan="7" class="text-center py-12 text-slate-400">এই ক্যাটাগরিতে কোনো অর্ডার পাওয়া যায়নি।</td></tr>
                ` : filteredOrders.map(o => `
                  <tr>
                    <td>
                      ...
                      <p class="text-[10px] text-slate-400 mt-0.5">${new Date(o.created_at).toLocaleDateString()}</p>
                    </td>

                    <td>
                      <h4 class="font-bold text-slate-900 dark:text-white">${o.customer_name}</h4>
                      <p class="font-mono text-slate-500">${o.phone}</p>
                      <p class="text-[10px] text-slate-400 truncate max-w-[150px]">${o.shipping_address}</p>
                    </td>

                    <!-- 🛡️ Dedicated Courier Fraud Check Button -->
                    <td>
                      <button onclick="window.FraudModal.open('${o.phone}', '${o.order_id}')" class="px-2.5 py-1 rounded-lg bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-800 hover:bg-rose-100 text-[11px] font-bold flex items-center gap-1">
                        <i data-lucide="shield-alert" class="w-3.5 h-3.5"></i> হিস্ট্রি চেক
                      </button>
                    </td>

                    <td>
                      <h4 class="font-black text-sm text-slate-900 dark:text-white">৳${o.total}</h4>
                      <span class="badge-info text-[9px] uppercase font-bold">${o.payment_method || 'COD'}</span>
                    </td>
                    ...
                    <td class="font-mono">
                      ...
                      ` : `
                        <span class="text-slate-400 text-[10px]">অ্যাসাইন হয়নি</span>
                      `}
                    </td>

                    <td>
                      <select onchange="window.updateOrderStatus('${o.order_id}', this.value)" class="px-2 py-1 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-xs font-bold outline-none">
                        ...
                        <option value="shipped" ${o.order_status === 'shipped' ? 'selected' : ''}>কুরিয়ারে পাঠানো</option>
                    ...
                    <td class="text-right">
                      <a href="/admin/orders/detail/${o.order_id}" class="p-2 inline-block text-slate-400 hover:text-brand-500">
...
window.updateOrderStatus = async (orderId, newStatus) => {
  try {
    ...
    store.showToast(`অর্ডার স্ট্যাটাস সফলভাবে '${newStatus}'-এ পরিবর্তিত হয়েছে!`, "success");
  } catch (err) {
    store.showToast(`স্ট্যাটাস পরিবর্তন ব্যর্থ: ${err.message}`, "danger");
  }
};
