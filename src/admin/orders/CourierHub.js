    ...
    <div class="min-h-screen flex bg-slate-50 dark:bg-luxury-dark font-bengali">
      ...
      <main class="flex-1 p-6 sm:p-10 max-w-7xl mx-auto overflow-y-auto">
        ...
        <div class="pb-6 mb-8 border-b border-slate-200 dark:border-slate-800">
          <span class="badge-info text-xs mb-1">কুরিয়ার প্লাগইন হাব</span>
          <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white">কুরিয়ার সার্ভিস ইন্টিগ্রেশন</h1>
          <p class="text-xs text-slate-500 mt-1">API কী, সিক্রেট এবং অটোমেটেড কুরিয়ার ট্র্যাকিং রুলস কনফিগার করুন</p>
        </div>
              ...
              <div>
                <div class="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-800 mb-4">
                  <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-xl bg-brand-500/10 text-brand-500 flex items-center justify-center font-bold">
                      <i data-lucide="${c.logo}" class="w-5 h-5"></i>
                    </div>
                    <div>
                  ...
                  <span class="${c.isActive ? 'badge-success' : 'badge-danger'} text-[10px]">
                ...
                <div class="space-y-3">
                  <div>
                    ...
                    <input type="password" value="${c.apiKey}" class="w-full px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-xs font-mono outline-none" />
                  </div>
                  <div>
                    <label class="block text-[11px] font-bold text-slate-400 mb-1">Secret Key</label>
                    <input type="password" value="${c.secretKey}" class="w-full px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-xs font-mono outline-none" />
                  </div>
                </div>
              </div>

              <div class="pt-4 border-t border-slate-100 dark:border-slate-800 flex gap-2">
                <button onclick="window.store.showToast('${c.name} টেস্ট কানেকশন সফল!', 'success')" class="flex-1 btn-secondary py-2 text-xs">
                  টেস্ট কানেকশন
                </button>
                <button onclick="window.store.showToast('${c.name} সেটিংস সেভ হয়েছে!', 'success')" class="flex-1 btn-primary py-2 text-xs">
                  সেভ করুন
                </button>
              </div>
